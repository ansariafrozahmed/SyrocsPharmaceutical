...
export default function OralSlider() {
  const [oralProducts, setOralProducts] = useState();

  useEffect(() => {
    const fetchOralProducts = async () => {
      const response = await fetch(`${ADMINURL}api/getProductsByCategory/oral`);
      const result = await response.json();
      // console.log(result, "IN SLIDER");
      setOralProducts(result);
    };
    fetchOralProducts();
  }, []);
